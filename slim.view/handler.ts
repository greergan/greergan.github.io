...
export function do_handler(model:slim.types.iKeyValueAny, handler:SlimViewHandler): Array<slim.types.iKeyValueAny> {
	let node:Array<slim.types.iKeyValueAny> = [];
	if(handler.is_valid()) {
		switch(handler.handler.toLowerCase()) {
			case 'equal':
				node = model.filter((member:slim.types.iKeyValueAny, index:number) => {
...
export function get_handler(handler_string:string):SlimViewHandler {
	SlimConsole.debug({message:"entry"}, handler_string);
...
SlimConsole.debug({message: "left_right_match"}, left_right_match);
...
//new trace({message:'entry', value: "/" + handler_string + "/"});
	...
	SlimConsole.debug({message:"handler_array", value:handler_array});
	const handler:SlimViewHandler = get_handler(handler_array[0])
	if(handler.is_valid()) {
		handlers.push(handler);
	}
	else if(handler.is_complex()) {
		SlimConsole.debug({message:"handler.is_complex()"})
	}
	SlimConsole.trace();
	return handlers;
}